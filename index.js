...
Sudoku.prototype.initMenu = function() {
  document.addEventListener('click',this.handleClick);
}

Sudoku.prototype.createPopupMenu = function(x,y) {
  //get width and height and /2 and define top and bottom
  const menu = document.getElementById("menu");
  menu.style.display = "block";
}

Sudoku.prototype.hidePopupMenu = function(){
...
Sudoku.prototype.handleMenuClick = function(e) {
  ...
  if (id==='Cancel'){
    ...
    //get current cell
    //populate and check for valid value
  }
}

Sudoku.prototype.handleClick = function(e) {
  console.log(e);
  if(/cell/.test(e.target.className)){
    console.log('cell block gets called');
    this.createPopupMenu(e.clientX,e.clientY);
  }
  //if target is a cell then show menu
}
...
Sudoku.prototype.handleDifficulty = function() {
  ...
  document.getElementById("difficulty").textContent =
    difficultyLabels[this.difficulty];
  this.handleNew();
};
...
Sudoku.prototype.isValidMove = function(input, x, y) {
  for (let i = 0; i < this.sudokuBlock; i++) {
    if (input === this.boardState[x][i] && i != y) return false;
  }
  for (let i = 0; i < this.sudokuBlock; i++) {
    if (input === this.boardState[i][y] && i != x) return false;
  }
  ...
  for (let i = 0; i < this.n; i++) {
    for (let j = 0; j < this.n; j++) {
      if (
        input === this.boardState[boardX * this.n + i][boardY * this.n + j] &&
        boardX * this.n + i !== x &&
        boardY * this.n + j !== y
      )
        return false;
